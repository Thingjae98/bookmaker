...
### 서비스 설명
- **ARCHIVE**: SweetBook Book Print API를 활용한 크리에이터/개발자를 위한 프리미엄 프로젝트 포트폴리오 북 제작 플랫폼
- 단일 서비스 `archive` — 프로젝트 스크린샷, 아키텍처 다이어그램, 회고 텍스트를 하드커버 북으로 아카이빙
- 2026-04-06 피벗: 기존 6개 서비스(baby, kindergarten 등) → 단일 `archive` 서비스로 전면 개편
...
5. **더미 데이터** — 실행 즉시 콘텐츠가 꽉 차 보이도록 src/data/dummy.js에 샘플 데이터 포함 (24페이지 포트폴리오 시나리오)
