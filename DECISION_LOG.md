...
## ADR-14 — AI 텍스트 일괄 생성: Gemini Vision 이미지 분석

### 배경
기존 AI TEXT 버튼은 페이지별 개별 생성만 지원. 26페이지를 하나씩 클릭하는 것은 비효율적. 또한 이미지를 분석하지 않고 메타데이터만으로 텍스트를 생성하여 실제 그림 내용과 무관한 해설이 생성됨.

### 결정
Gemini 2.5 Flash의 Vision 기능으로 모든 내지 이미지를 분석하여 작품 해설 + 제목을 일괄 생성하는 `✨ AI 텍스트 일괄 생성` 버튼 추가.

### 기술적 결정 — 5장씩 배치 분할
- **문제**: 26장 이미지(~8.7MB) → base64(~11.6MB)를 한 번에 전송하면 Next.js API Route의 body size 제한 초과
- **해결**: 프론트엔드에서 5장씩 배치로 나눠 6회 API 호출, 서버에서 각 배치를 Gemini Vision에 전송
- **대안 검토**: Claude API도 26장 처리 가능(최대 100장)하나, Gemini가 영구 무료 티어(250 RPD) 제공하여 채택

### 구현
- `POST /api/generate-batch-text` — 배치 수신 → Gemini Vision 호출 → JSON 배열 반환
- `editor/page.jsx` — `handleBatchAiText()` 핸들러 + 진행률 표시(배치 N/6)
- 실패 배치는 폴백 텍스트로 자동 채움 → 전체 실패 없이 부분 성공 가능

### 결과
- 26페이지 전체 텍스트 생성 시간: ~90초 (6배치 × ~15초)
