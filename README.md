...
- **미니 와이어프레임 UI**: 갤러리 모달의 템플릿 선택 카드에 API 미리보기 이미지가 없을 경우, Tailwind CSS로 해당 레이아웃을 직관적으로 표현한 미니 와이어프레임(사진 영역·텍스트 영역·달력 그리드 등 6종)을 자동 렌더링 → 선택 전 레이아웃 예측 가능
...
- **판형별 최소 장수 및 증분 단위 완벽 지원**: 선택한 판형의 pageMin(최소 페이지)과 pageIncrement(증분 단위)를 자동 계산하여 규칙 위반으로 인한 400 에러 원천 차단
