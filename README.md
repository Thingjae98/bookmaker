...
- **다중 이미지 Drag & Drop 업로드 + 갤러리 UI**: 에디터 하단 갤러리 섹션에서 여러 사진을 한 번에 업로드, 썸네일 그리드로 표시, 드래그로 순서 변경
- **표지 직접 지정**: 갤러리 썸네일 클릭 → 모달에서 앞표지·뒤표지·내지 역할 지정 — 자동 지정 없음, 중복 방지 Validation
- **템플릿 동적 분기**: 텍스트 입력 여부에 따라 템플릿 자동 선택 (텍스트 있음 → cnH0Ud1nl1f9 사진+텍스트형, 없음 → 6dJ0Qy6ZmXej 이미지 전용)
- **가로형 사진 양면(Spread) 분할**: 갤러리 모달에서 체크박스 선택 → Canvas API로 좌/우 반분할 → 연속된 2페이지로 전송 (JPEG quality=1.0 최고 화질)
