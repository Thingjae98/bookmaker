...
- **실시간 스프레드 미리보기**: 에디터에서 구성한 실제 표지·내지 데이터를 기반으로 책을 펼친 모습의 스프레드 뷰(Spread View)를 렌더링합니다. 첫 스프레드는 `[뒤표지 | 앞표지]`, 이후는 `[내지 1 | 내지 2]` 쌍으로 표시하며, CSS 책등(Spine) 그림자 효과로 실제 책의 느낌을 재현합니다
...
- **블러 스프레드 미리보기**: 상위 3스프레드(6페이지)는 선명, 나머지는 블러 처리 → 구매 유도 오버레이
